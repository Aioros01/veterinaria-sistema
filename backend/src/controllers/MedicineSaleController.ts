...
import { MedicineSale, SaleStatus, PurchaseLocation } from '../entities/MedicineSale';
import { Medicine } from '../entities/Medicine';
import { Prescription, PurchaseStatus } from '../entities/Prescription';
import { User } from '../entities/User';
import { Pet } from '../entities/Pet';

export class MedicineSaleController {
  private saleRepository = AppDataSource.getRepository(MedicineSale);
  ...
  async createSaleFromPrescription(req: Request, res: Response): Promise<Response> {
    try {
      const {
        prescriptionId,
        purchaseLocation,
        quantity,
        quantityInClinic,  // Nueva: cantidad a comprar en clínica
      ...
      if (purchaseLocation === PurchaseLocation.SPLIT) {
      ...
      } else if (purchaseLocation === PurchaseLocation.IN_CLINIC) {
      ...
      prescription.purchaseStatus =
        finalPurchaseLocation === PurchaseLocation.IN_CLINIC ? PurchaseStatus.PURCHASED_IN_CLINIC :
        finalPurchaseLocation === PurchaseLocation.EXTERNAL ? PurchaseStatus.PURCHASED_EXTERNAL :
        PurchaseStatus.PURCHASED_IN_CLINIC; // SPLIT se marca como comprado en clínica porque hubo venta parcial
      ...
      return res.status(201).json({
        message: finalPurchaseLocation === PurchaseLocation.SPLIT
        ...
        splitDetails: finalPurchaseLocation === PurchaseLocation.SPLIT ? {
          quantityInClinic: finalQuantityInClinic,
          quantityExternal: finalQuantityExternal,
  ...
  async createDirectSale(req: Request, res: Response): Promise<Response> {
    try {
      const {
        medicineId,
        clientId,
      ...
      const sale = this.saleRepository.create({
        ...
        unitPrice: medicine.unitPrice || 0,
        discountPercentage,
        purchaseLocation: PurchaseLocation.IN_CLINIC,
        notes,
        status: SaleStatus.COMPLETED,
        veterinarianId: (req as any).user?.id,
        createdBy: (req as any).user?.id,
        updatedBy: (req as any).user?.id
      });
  ...
  // Obtener ventas
  async getSales(req: Request, res: Response): Promise<Response> {
    try {
      const {
        clientId,
        petId,
  ...
  async getSalesSummary(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const parameters: any = { status: SaleStatus.COMPLETED };
  ...
  async cancelSale(req: Request, res: Response): Promise<Response> {
    try {
      ...
      sale.updatedBy = (req as any).user?.id;
      await this.saleRepository.save(sale);

      // Si tiene prescripción, actualizar su estado
      if (sale.prescription) {
        sale.prescription.purchaseStatus = PurchaseStatus.NOT_PURCHASED;
